...
interface Props {
  ...
  totalVotes: number;
  pollID: string;
  setPolls?: React.Dispatch<
    React.SetStateAction<
      Poll[] | (Poll | Announcement)[] | (Post | Poll | Announcement)[]
    >
  >;
}

const Option = ({ option, totalVotes, pollID, setPolls }: Props) => {
  ...
  const handleVoteOption = async (
    option: Option,
    setIsVoted: React.Dispatch<React.SetStateAction<boolean>>
  ) => {
    await semaphore.acquire();

    const URL = `/poll/vote/${pollID}/${option.id}`;
  ...
  const handleUnVoteOption = async (
    option: Option,
    setIsVoted: React.Dispatch<React.SetStateAction<boolean>>
  ) => {
    await semaphore.acquire();

    const URL = `/poll/unvote/${pollID}/${option.id}`;
                ...
                options: p.options.map((o) => {
                  if (o.id == option.id)
                    return {
                      ...o,
                      noVotes: o.noVotes - 1,
                      votedBy: (o.votedBy || []).filter((u) => u.id != userID),
                    };
                  return o;
                }),
              };
  ...
  return (
    <div className="w-full flex flex-col gap-1">
            ...
            .map((u, index) => {
              return (
                <Image
                  ...
                  className={`w-6 h-6 rounded-full cursor-default absolute top-0 right-${
                    index * 3
                  } animate-fade_third`}
                />
              );
      ...
      <div
        onClick={() => {
          isVoted
            ? handleUnVoteOption(option, setIsVoted)
            : handleVoteOption(option, setIsVoted);
        }}
      ...
      >
        <div
          ...
          className={`h-full ${
            isVoted
              ? "bg-[#9275b9]"
              : "bg-[#e7d5ffba] group-hover:bg-[#cca5ffba]"
          } rounded-lg ${

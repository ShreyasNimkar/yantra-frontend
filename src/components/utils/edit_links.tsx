...
const Links = ({
  ...
  const addLink = (el?: FormEvent<HTMLFormElement>) => {
    ...
    if (isURL(newLink)) {
      if (newLink.startsWith("https://www."))
        setLinks((prev) => [...prev, newLink]);
      else if (newLink.startsWith("https://"))
        setLinks((prev) => [
          ...prev,
          newLink.replace("https://", "https://www."),
        ]);
      else if (newLink.startsWith("www."))
        setLinks((prev) => [...prev, "https://" + newLink]);
            ...
            <div className="flex flex-col gap-4">
              {links.map((link: string, index: number) => {
                return (
                  <div
                    key={index}
                    className="w-full h-8 flex justify-between gap-2 items-center font-Inconsolata"
                  >
                    <div
          ...
          {links.length < maxLinks ? (
            <form onSubmit={addLink}>
              <input
                ...
                value={newLink}
                onChange={(el) =>
                  setNewLink(el.target.value.toLowerCase().trim())
                }

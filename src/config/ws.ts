...
class SocketService {
  ...
  public connect(userID: string | undefined = Cookies.get("id")): void {
    if (!this.socket) {
      ...
      this.socket = new WebSocket(
        `${SOCKET_URL}?userID=${userID}&token=${token}`
      );
  ...
  public sendMessage(
    ...
    if (this.socket) {
      const outgoingMessageEvent = new SendMessageEvent(
        message,
        chatID,
        userID,
        self
  ...
  public sendNotification(userID: string, content: string) {
    if (this.socket) {
      const outgoingNotificationEvent = new SendNotificationEvent(
        userID,
        content
      );
  ...
  public sendReadMessage(userID: string, messageID: string, chatID: string) {
    if (this.socket) {
      const outgoingNotificationEvent = new SendMessageReadEvent(
        userID,
        messageID,
        chatID
      );
  ...
  public setupGroupChatListRoutes(
        ...
        switch (event.type) {
          case "new_message":
            ...
            if (
              ...
              if (
                store.getState().messaging.currentChatID == "" &&
                store.getState().messaging.currentGroupChatID == ""
              )
                // store.dispatch(
                //   setUnreadChats([...unreadChatIDs, messageEventPayload.chatID])
                // );
                toast.info(
                  "New Message from: " + messageEventPayload.user.name,
                  {

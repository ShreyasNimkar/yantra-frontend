...
const NewResourceFile = ({
  ...
  const handleAddFile = async () => {
    ...
    const URL = `/resource/${resourceBucketID}/file`;
    ...
    <>
      <div className="w-[60%] font-poppins absolute bg-white border-2 border-primary_text shadow-xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 p-6 rounded-xl flex flex-col gap-4 animate-fade_third">
        ...
        <div className="w-full h-fit flex flex-col gap-4">
          ...
          />
          <TextArea
            label="Resource File Description"
            val={description}
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (files && files.length > 0) {
              ...
              if (file.size > 5 * 1024 * 1024)
                Toaster.error("File cannot be larger than 5 MB");
        ...
        <div className="flex flex-col gap-4 items-center mt-4">
          ...
          <div className="w-full flex items-center justify-between">
            ...
            <div className="w-[10%] text-center text-sm max-lg:text-xs text-gray-400">
              or
            </div>
            <div className="w-[45%] h-[1px] bg-gray-200"></div>
          </div>
